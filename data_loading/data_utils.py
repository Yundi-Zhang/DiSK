...
class NormalLines_Sampler(object):
    """Undersampling strategy to generate a random mask with a given acceleration factor. The dimension of the mask is determined by the mask_dim parameter. The mask is generated by randomly selecting points in the data space based on normal distribution."""
    
    def __init__(self, acc, data_shape, seed=None, **kwargs):
        assert len(data_shape) == 4, "Data shape should be 4D."
        self.data_dim = 4
        self.seed = seed
        self.acc = acc
    ...
    def get_mask(self):
        ...
        self.lines_num = int(Ny * N / self.acc)
        ...
        for i in range(N):
            nd_indices = np.random.choice(Ny, self.lines_num // N, replace=False, p=normal_p)
            mask[:, nd_indices, i] = 1
        rand_lines_num = self.lines_num - self.lines_num // N * N
        if rand_lines_num > 0:
...
class SimulateCartesian(object):
    ...
    def _addSynthethicPhase(self, img):
        ...
        smoother = self._fspecial((img.shape[-2], img.shape[-1]), max(1.0, round(img.shape[-2] / 200)))  # TODO: Ask Kerstin what this 200 is for, had to add min(1.0,...)
...
def normal_dist(pos , mean , cov):
    ...
    Args:
        x (int or nd.array or torch.tensor): qurey values. When x is a vector, the function returns the probability density of each element in x. When x is a matrix, the function returns the probability density of each column of x.
